...
function nextPage() {
  ...
  if(getExperimentStage() === 0) {
    ...
    // Show new description. This is valid for exp2.
    //$('#description').text(getDescriptionToShow()['description']);

    // Show graph for a few seconds. This is valid for exp2_rev2.

        // First, get the datapoints of the plot_number
        datapoints = getDatapoints(getDescriptionToShow()['plot_number']);

        // Update instructions (which includes Starting a timer)
